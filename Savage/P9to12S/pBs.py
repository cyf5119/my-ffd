...
special_actions[33259] = raid_utils.donut_shape(2, 9) #simple_cast 特弥斯 cast 幻影黑暗#33259 shape:0x138e3 time:2.80
special_actions[33271] = raid_utils.donut_shape(8, 50) #simple_cast 特弥斯 cast 黑暗回环#33271 shape:0x128f5 time:3.20
special_actions[34697] = raid_utils.donut_shape(8, 50) #simple_cast 特弥斯 cast 外暗#34697 shape:0x128f5 time:10.20
special_actions[33317] = raid_utils.donut_shape(8, 50) #simple_cast 特弥斯的幻影 cast 黑暗回环#33317 shape:0x128f5 time:3.20
special_actions[34694] = 0 # 驳回否决
special_actions[34695] = 0 # 驳回否决
delay_until[33263] = 8 #simple_cast 特弥斯 cast 光爆#33263 shape:0x20000 time:10.50
delay_until[33264] = 8 #simple_cast 特弥斯 cast 暗黑冲击#33264 shape:0x20000 time:10.50
delay_until[33265] = 8 #simple_cast 特弥斯 cast 暗黑冲击#33265 shape:0x20000 time:10.50
delay_until[33309] = 8 #simple_cast 特弥斯的幻影 cast 光爆#33309 shape:0x20000 time:12.10
delay_until[33310] = 8 #simple_cast 特弥斯的幻影 cast 暗黑冲击#33310 shape:0x20000 time:12.10
delay_until[33311] = 8 #simple_cast 特弥斯的幻影 cast 暗黑冲击#33311 shape:0x20000 time:12.10
delay_until[34742] = 8 #simple_cast 特弥斯 cast 光爆#34742 shape:0x20000 time:12.00
delay_until[34743] = 8 #simple_cast 特弥斯 cast 暗黑冲击#34743 shape:0x20000 time:12.00
delay_until[34744] = 8 #simple_cast 特弥斯 cast 暗黑冲击#34744 shape:0x20000 time:12.00
...
dark_violet_surface = col.purple_surface
dark_violet_line = col.purple_line
pale_goldenrod_surface = col.yellow_surface
pale_goldenrod_line = col.yellow_line

_light_actions = [33258, 33263, 34696, 33309, 34742, 33316]
#simple_cast 特弥斯 cast 幻影闪耀#33258 shape:0x10000 time:2.80
#simple_cast 特弥斯 cast 内光#34696 shape:0x10000 time:10.20
#simple_cast 特弥斯的幻影 cast 光爆破#33316 shape:0x10000 time:3.20
...
@pBs.on_cast(33258, 33270, 33263, 34696, 34742) # 光四四
#simple_cast 特弥斯 cast 光爆破#33270 shape:0x10000 time:3.20
@CallOnce
def on_cast_light_combo(evt: 'NetworkMessage[zone_server.ActorCast]'):
    ...
    for actor in raid_utils.iter_main_party(False):
        if raid_utils.is_class_job_healer(actor.class_job):
            ...
            raid_utils.draw_circle(radius=6, pos=actor, duration=cast_time, line_color=col.green_line, surface_color=0)
            # logger.debug(f'绘制了一次{actor.name}的44分摊')

# 八方后的光四四与暗二二都存在重复绘制的情况，故更改分摊圈

@pBs.on_cast(33259, 33271, 33264, 34697, 34743) # 暗二二
@CallOnce
def on_cast_dark_combo(evt: 'NetworkMessage[zone_server.ActorCast]'):
    ...
    for actor in raid_utils.iter_main_party(False):
        if raid_utils.is_class_job_dps(actor.class_job):
            ...
            raid_utils.draw_circle(radius=3, pos=actor, duration=cast_time, line_color=col.green_line, surface_color=None)
            # logger.debug(f'绘制了一次{actor.name}的22分摊')


@pBs.on_cast(33254, 33255) # 八方
...
@pBs.on_set_channel(249) # 连线，光分摊暗凶鸟冲
...
def on_cast_dismissal_overruling(evt: 'NetworkMessage[zone_server.ActorCast]'):
    raid_utils.draw_knock_predict_circle(radius=11, pos=raid_utils.NActor.by_id(evt.header.source_id), duration=evt.message.cast_time, knock_distance=11,
                                         line_color=glm.vec4(.2, 1, .2, .8), surface_color=glm.vec4(.6, 1, .6, .2))


upheld_ruling_rad = math.radians(10)


@pBs.on_lockon(156, 157) # 光明奔流
