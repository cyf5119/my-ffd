...
omen_color[33121] = glm.vec4(1, .2, .2, .5), glm.vec4(1, 1, 1, .8)
...
class DualSpell:
    ...
    def make_lightning(self, actor_id):
        ...
        return raid_utils.draw_rect(
            ...
            duration=self.dur,
            surface_color=glm.vec4(1, .6, 1, .2), line_color=glm.vec4(1, .2, 1, .8)
        )
    ...
    def make_ice(self):
        ...
        self.ice_omen = raid_utils.draw_circle(
            ...
            inner_radius=14,
            duration=self.dur,
            surface_color=glm.vec4(.6, 1, 1, .2), line_color=glm.vec4(.2, 1, 1, .8)
        )
    ...
    def draw(self, actor: raid_utils.NActor, draw_type, dur):
        ...
        if draw_type == self.circle:
            return raid_utils.draw_circle(
                ...
                pos=actor_pos,
                duration=dur,
                surface_color=glm.vec4(1, .2, .2, .5), line_color=glm.vec4(1, 1, 1, .8)
            )
...
class Levinstrike:
    ...
    def draw_ball(self, idx, dur):
        ...
        return raid_utils.draw_circle(radius=6, pos=target_pos, duration=dur,
                                      line_color=glm.vec4(1, .2, 1, .8), surface_color=glm.vec4(1, .6, 1, .2))
    ...
    def draw_fire(self, idx, dur):
        ...
        return raid_utils.draw_distance_line(
            ...
            line_color=glm.vec4(1, .2, .2, .8), surface_color=glm.vec4(1, .6, .6, .2)
        )
    ...
    def draw_ice(self, idx, dur):
        ...
        return raid_utils.draw_circle(radius=20, pos=raid_utils.NActor.by_id(self.ice[idx]), duration=dur,
                                      line_color=glm.vec4(.2, 1, 1, .8), surface_color=glm.vec4(.6, 1, 1, .2))
    ...
    def on_cast_thunderbolt(self, evt: NetworkMessage[zone_server.ActorCast]):
        ...
        def _draw(_i):
            raid_utils.draw_fan(
                ...
                duration=evt.message.cast_time + .5,
                line_color=glm.vec4(1, .2, 1, .8), surface_color=glm.vec4(1, .6, 1, .2)
            )
