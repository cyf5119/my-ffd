...
omen_color[33567] = glm.vec4(1, .2, .2, .5), glm.vec4(1, 1, 1, .8)
...
def play_boa(x, y, z, r, cast_time, dif, dis, rad, times, show_time=4):
    ...
    for i in range(1, times):
        ...
        raid_utils.draw_circle(
            ...
            pos=glm.vec3(x, y, z),
            duration=show_time,
            surface_color=glm.vec4(1, .2, .2, .5),
            line_color=glm.vec4(1, 1, 1, .8)
        )
...
def on_channel_factor_in(evt: 'ActorControlMessage[actor_control.SetChanneling]'):
    raid_utils.timeout_when_channeling_change(raid_utils.draw_circle(radius=21.5, pos=raid_utils.NActor.by_id(evt.param.target_id), duration=10), evt)
    # 截线的特效感觉需要调大一些
