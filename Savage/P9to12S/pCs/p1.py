...
special_actions[33540] = 0 # 论灵魂 全屏AOE

omen_color[33521] = glm.vec4(1, 1, .6, .2), glm.vec4(1, 1, .2, .8) # 33521 飞光 8.7s 小怪光直线
omen_color[33522] = glm.vec4(1, .6, 1, .2), glm.vec4(1, .2, 1, .8) # 33522 影伤 8.7s 小怪暗直线
# 小怪光暗连线改一下颜色在三范好认一些
omen_color[33518] = glm.vec4(1, .2, .2, .5), glm.vec4(1, 1, 1, .8) # 光波 小怪双四分之一场地直线
omen_color[33511] = glm.vec4(1, .2, .2, .5), glm.vec4(1, 1, 1, .8) # 33511 三魂一体 9.7s
omen_color[33512] = glm.vec4(1, .2, .2, .5), glm.vec4(1, 1, 1, .8) # 33512 三魂一体 9.7s
omen_color[33505] = glm.vec4(1, .2, .2, .5), glm.vec4(1, 1, 1, .8) # 三魂一体
omen_color[33506] = glm.vec4(1, .2, .2, .5), glm.vec4(1, 1, 1, .8) # 三魂一体
# 三魂一体的四种读条
# 33550 灵极进升 1.7s 黑塔白塔
# 33551 星极进升 1.7s 黑塔白塔
...
class TrinityOfSouls:
    ...
    def apply_omen(self, source_actor: 'raid_utils.NActor', facing, is_reverse, next_idx, dur=2.6):
        ...
        raid_utils.draw_fan(degree=180, radius=60, pos=source_actor, facing=facing, duration=dur,
                            surface_color=glm.vec4(1, .2, .2, .5), line_color=glm.vec4(1, 1, 1, .8)) # 三魂一体的后面两次
    ...
    def draw_anthropos(self, idx, dur):
        ...
        if self.anthropos_order[idx] == 4564:  # light
            raid_utils.draw_rect(
                ...
                duration=dur, surface_color=glm.vec4(1, 1, .6, .2), line_color=glm.vec4(1, 1, .2, .8)
            )
            raid_utils.draw_rect(
                width=4, length=100,
                pos=center,
...
# 连线地板危险区
@pCs.on_set_channel(232)
def on_channeling_ground(msg: ActorControlMessage[actor_control.SetChanneling]):
    pos = raid_utils.NActor.by_id(msg.source_id).pos
    raid_utils.timeout_when_channeling_change(raid_utils.draw_rect(width=20, length=5, arg=1, pos=pos, facing=0, duration=10,
                                                                   surface_color=glm.vec4(1, .2, .2, .5), line_color=glm.vec4(1, 1, 1, .8)), msg)


# 放塔提示
@pCs.on_add_status(3580, 3579)
def on_add_status_tower(evt: ActorControlMessage[actor_control.AddStatus]):
    actor = raid_utils.NActor.by_id(evt.source_id)
    if raid_utils.assert_status(actor, evt.param.status_id, 5):
        color = glm.vec4(.5, .1, 0, .3) if evt.param.status_id == 3580 else glm.vec4(.9, 1, 1, .3)
