...
class col():
    ...
    white_line = glm.vec4(1, 1, 1, .8)
    highlight_red_surface = glm.vec4(1, .2, .2, .5)
    guide_line = glm.vec3(.2, 1, .2)
    guide_surface = glm.vec3(.6, 1, .6)

omen_color[33121] = col.highlight_red_surface, col.white_line
    ...
    def on_cast_thunderbolt(self, evt: NetworkMessage[zone_server.ActorCast]):
        ...
        def _draw(_i):
            raid_utils.draw_fan(
                ...
                duration=evt.message.cast_time + .5, # 多半秒等特效出
                surface_color=col.purple_surface, line_color=col.purple_line
            )
